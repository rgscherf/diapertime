...
(defn- new-events
  ...
  (if user
    (do
      (mcoll/insert db/database db/babies (dissoc new-event :_id))
...
(defn- new-events-and-summary
  ...
  (let [realized-new-event (new-event-from-deltas new-event)
        ...
        new-events (new-events user events realized-new-event)]
    {:events new-events
...
(comment
  ...
  (new-events-and-summary test-data)
  (clojure.repl/pst)
  (time/before? (time/today-at-midnight) (time/now))
  (time/in-minutes
    (time/interval
       (time/now) (time/today-at-midnight))))
