...
(defn- new-events
  ...
  Else (eg if we're looking at the random events) just conj the new event.
  In either case, adds metrics to new events list."
  [user events new-event]
  (metrics/add-metrics
    (if user
      (do (persist-event! user new-event)
          (:events (mcoll/find-one-as-map db/database
                                          db/babies
                                          {:email (:email user)})))
