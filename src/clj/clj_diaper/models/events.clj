...
(defn- persist-event!
  [user new-event]
  (do
    (mcoll/update db/database
                  db/babies
                  {:email (:email user)}
                  (update (mcoll/find-one-as-map db/database
                                                 db/babies
                                                 {:email (:email user)})
                          :events
                          conj new-event))
...
(comment
  ...
  (filter #(:feed %) (mcoll/find-maps db/database db/babies))
  (mcoll/drop db/database db/babies)
  (clojure.repl/pst)
  ;; basic testing
  (def hi {:hello "world"
           :events []})
  (update-in hi [:events] conj 2)
  ;; testing real records
  (def me (mcoll/find-one-as-map db/database db/babies {:email "rgscherf@gmail.com"}))
  (mcoll/update db/database
                db/babies
                {:email "rgscherf@gmail.com"})
  (mcoll/insert-and-return db/database
                           db/babies
                           {:email "test"
                            :_id (ObjectId.)
                            :events []})
  (def te (mcoll/find-one-as-map db/database db/babies {:email "test"}))
  (mcoll/update db/database
                db/babies
                {:email "test"}
                (update (mcoll/find-one-as-map db/database
  ...
  (persist-event!
    (mcoll/find-one-as-map db/database db/babies {:email "rgscherf@gmail.com"})
    (new-event-from-deltas (:new-event test-data)))
  ;; testing john
  (mcoll/insert db/database db/babies {:first "John" :last "Connor" :_id (ObjectId.)})
  (def john (mcoll/find-one-as-map db/database db/babies {:first "John"}))
  (mcoll/update-by-id db/database
                      db/babies
                      (:_id john)
                      (assoc john :dog "buddy")))
                      ; {:_id (:_id john)
                      ;  :first "John"
                      ;  :last "Connot"
                      ;  :middle "Figgle"}))
...
(defn- new-events
  "Make new events list as a result of adding a new event.
  If user is provided, new event will be inserted into DB and new event list returned.
  Else (eg if we're looking at the random events) just conj the new event and calc new metrics."
  [user events new-event]
  (if user
    (metrics/add-metrics
      (persist-event! user new-event))
    (->> (conj (date-typed-events events)
               new-event)
         (map #(dissoc % :metrics))
         metrics/add-metrics)))
